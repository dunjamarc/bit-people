        ...
        return (
            <ul className='collection'>
                ...
                className={[ 'collection-item avatar', ((this.props.value.gender === 'female') ? 'female' : 'male')].join(' ')}>
                    ...
                    <p><i className="material-icons">cake</i>date of birth: {new Date(this.props.value.dob.date).getDate() +'.'+ (new Date(this.props.value.dob.date).getMonth() + 1) +'.'+ new Date(this.props.value.dob.date).getFullYear()}</p>
                </li>
            </ul>
        )
    }
